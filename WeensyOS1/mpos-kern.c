...
interrupt(registers_t *reg)
{
	...
	case INT_SYS_NEWTHREAD: {
		...
		{	
			...
			{
				memcpy(&(proc_array[p].p_registers), &(current->p_registers), sizeof(registers_t));
				proc_array[p].p_pid = p;
				proc_array[p].p_registers.reg_eax = 0;
				proc_array[p].p_registers.reg_eip = current->p_registers.reg_eax;
				proc_array[p].p_registers.reg_esp = p * PROC_STACK_SIZE + PROC1_STACK_ADDR;
				proc_array[p].p_state = P_RUNNABLE;
				proc_array[p].p_waiting = -1;
	...
	case INT_SYS_KILL: {
		...
		else 
		{
			...
			pid_t pid;
			if (proc_array[p].p_waiting > 0)
			{
				proc_array[proc_array[p].p_waiting].p_state = P_RUNNABLE;
				proc_array[p].p_waiting = -1;
			}
		/*for (pid = 1; pid < NPROCS; pid++)
		{
			if (proc_array[pid].p_waiting == p)
			{
				proc_array[pid].p_state = P_RUNNABLE;
				proc_array[pid].p_waiting = -1;
			}
		}*/
		}
		schedule();
