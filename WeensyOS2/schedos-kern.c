...
static unsigned int x = 123456;
static unsigned int y = 362092;
static unsigned int z = 895104;
...
start(void)
{
	...
	// Initialize proprity for each process
	proc_array[1].p_priority = PRIORITY_1; 
	proc_array[2].p_priority = PRIORITY_2;
	proc_array[3].p_priority = PRIORITY_3;
	proc_array[4].p_priority = PRIORITY_4;

	proc_array[1].p_share = SHARE_1; 
	proc_array[2].p_share = SHARE_2;
	proc_array[3].p_share = SHARE_3;
	proc_array[4].p_share = SHARE_4;

	proc_array[1].p_counter = COUNTER_1; 
	proc_array[2].p_counter = COUNTER_2;
	proc_array[3].p_counter = COUNTER_3;
	proc_array[4].p_counter = COUNTER_4;

	proc_array[1].p_ticket = TICKET_1; 
	proc_array[2].p_ticket = TICKET_2;
	proc_array[3].p_ticket = TICKET_3;
	proc_array[4].p_ticket = TICKET_4;

	// Initialize the cursor-position shared variable to point to the
	// console's first character (the upper left).
	cursorpos = (uint16_t *) 0xB8000;
	//mutex = (uint32_t) 0xB8500;
