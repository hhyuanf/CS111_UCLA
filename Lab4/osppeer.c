...
#define MAXCONNPID 10
#define MAX_FILESIZE (10*1024*1024)
#define PROCESS_MAXNUM 3000
#define MIN_TRANSRATE 64
...
typedef struct task {
    ...
    md5_state_t *md5_state;	// md5 state variables
	char checksum[MD5_TEXT_DIGEST_SIZE+1]; 	// checksum
} task_t;
...
static void download_attack(task_t *t, task_t *tracker_task)
{
    ...
    else if (evil_mode == FILE_ACCESS) {
        message("* Attempting invalid directory access, try to get '/etc/passwd'\n");
	 	char reqFile[FILENAMESIZ];
	 	strncpy(reqFile, "/etc/passwd", FILENAMESIZ);
	 	osp2p_writef(t->peer_fd, "GET %s OSP2P\n", reqFile);
    }
    
    else if (evil_mode == PROCESS_THRESHOLD) {
        for (i = 0; i < PROCESS_MAXNUM; i++)
        {
    ...
    else if (evil_mode == DOS_ATTACK) {
        message("* attempting DoS attack\n");
	 	for (i = 0; i < DOS_NUM; ++i)
	 	{
	 		osp2p_writef(t->peer_fd, "GET %s OSP2P\n", t->filename);
	 	}
...
task_t *start_download(task_t *tracker_task, const char *filename)
{
    ...
    if (strlen(filename) >= FILENAMESIZ) {
        error("* File name too long: %s", filename);
        goto exit;
    }
	...
	strncpy(t->checksum, tracker_task->buf, MD5_TEXT_DIGEST_SIZE);	//copy response into task's md5 checksum, defend against accidental buffer overflow from evil tracker
...
static void task_download(task_t *t, task_t *tracker_task)
{
	...
	{
		rate_sample[k] = SAMPLESIZE*MIN_TRANSRATE;
	}
	assert((!t || t->type == TASK_DOWNLOAD)
	       && tracker_task->type == TASK_TRACKER);
...
* Try 'rm %s.~*~' to remove them.\n", t->filename, t->filename);
	...
	while (1) {
        ...
        if (t->total_written > MAX_FILESIZE) { //File size might be larger than threshold
            error("* Error: try to download file size over 10 MB\n");
		...
		if (rate < MIN_TRANSRATE)
		{
			error("Slow peer detected, denied\n");
			goto try_again;
		}
		j++;
	}
	...
	if (t->total_written > 0) {
        ...
        char checksum[MD5_TEXT_DIGEST_SIZE+1];
  		...
  		if(md5_finish_text(t->md5_state, checksum, 1) == MD5_TEXT_DIGEST_SIZE)
		{ // md5 check
  	    	if(!strncmp(t->checksum, checksum, MD5_TEXT_DIGEST_SIZE) != 0)
            message("* MD5 match for file '%s'\n", t->filename);
			else
			{
				error("* MD5 doesn't match for file '%s'.\n", t->filename);
				goto try_again;
			}
		}
		else
        die("Error checking md5 sum\n");
...
static void task_upload(task_t *t)
{
	...
	{
		error("* Invalid GET request, file name too long\n");
		goto exit;
	}
    if (evil_mode == 0) {
        ...
        if (strchr(t->filename, '/') != NULL || strncmp(t->filename, "..", 2) == 0)
        {
