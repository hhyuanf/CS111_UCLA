...
ospfs_dir_readdir(struct file *filp, void *dirent, filldir_t filldir)
{
	...
	while (r == 0 && ok_so_far >= 0 && f_pos >= 2) {
		...
		od = (ospfs_direntry_t *)ospfs_inode_data(dir_oi, OSPFS_DIRENTRY_SIZE*(f_pos-2));
...
ospfs_unlink(struct inode *dirino, struct dentry *dentry)
{
	...
	if (oi->oi_ftype == OSPFS_FTYPE_SYMLINK) {
		sym_oi = (ospfs_symlink_inode_t *) oi;
		sym_oi->oi_size = 0;
		sym_oi->oi_symlink[0] = 0;
	}
...
allocate_block(void)
{
	...
	uint32_t total = ospfs_super->os_nblocks;
	uint32_t i;	
	for (i = 0; i < total; i++) {
...
free_block(uint32_t blockno)
{
	/* EXERCISE: Your code here */
	void *bitmap = ospfs_block(OSPFS_FREEMAP_BLK);
	uint32_t firstBlk = ospfs_super->os_firstinob + (ospfs_super->os_ninodes)/OSPFS_BLKINODES;
	if (blockno < ospfs_super->os_nblocks && blockno >= firstBlk)	
...
add_block(ospfs_inode_t *oi)
{
	...
	/* EXERCISE: Your code here */
	if (indir2_index(n) == 0) {//indirect2
		int32_t offset[2];
		uint32_t *ptr1 = NULL;
		uint32_t *ptr2 = NULL;
		uint32_t in2block[3] = {0, 0, 0};
		offset[0] = direct_index(n);
		offset[1] = indir_index(n);
		if (oi->oi_indirect2 == 0) {
			in2block[0] = allocate_block();
			if (in2block[0] == 0) 
				return -ENOSPC;
			allocated[0] = ospfs_block(in2block[0]);
			oi->oi_indirect2 = in2block[0];
			memset(ospfs_block(in2block[0]), 0, OSPFS_BLKSIZE);
		}
		if (offset[0] == 0) { //doubly indirect but empty
			in2block[1] = allocate_block();
			if (in2block[1] == 0) {
				if (allocated[0] != 0) 
			...
			ptr2[offset[1]] = in2block[1];
			memset(ospfs_block(in2block[1]), 0, OSPFS_BLKSIZE);
			if (allocated[0] == 0) 
				allocated[0] = ospfs_block(in2block[1]);
			else 
				allocated[1] = ospfs_block(in2block[1]);
		}
		if ((in2block[2] = allocate_block()) == 0) {
			if (allocated[1] != 0) {
				free_block(in2block[0]);
				free_block(in2block[1]);
				return -ENOSPC;
			}		
		...
		ptr2 = ospfs_block(oi->oi_indirect2);
		ptr1 = ospfs_block(ptr2[offset[1]]);
		ptr1[offset[0]] = in2block[2];
		memset(ospfs_block(in2block[2]), 0, OSPFS_BLKSIZE);	
	}
	else if (indir_index(n) == 0) {
		uint32_t offset = direct_index(n);
		uint32_t *indBlk = NULL;
		uint32_t block;
		//uint32_t i;
		if (oi->oi_indirect == 0) {
			allocated[0] = (int *)allocate_block();
			//eprintk("%d\n", i);
			
			if (allocated[0] == 0) 
				return -ENOSPC;
			oi->oi_indirect = (uint32_t)allocated[0];
			indBlk = ospfs_block(oi->oi_indirect);
			memset(indBlk, 0, OSPFS_BLKSIZE);
		} 
		else
			indBlk = ospfs_block(oi->oi_indirect);
		...
		if (block == 0 && allocated[0] != 0) {
			free_block((uint32_t)allocated[0]);
			oi->oi_indirect = 0;
			return -ENOSPC;
		}
		indBlk[offset] = block;
...
remove_block(ospfs_inode_t *oi)
{
	...
	/* EXERCISE: Your code here */
	int32_t offset[2];
	offset[0] = direct_index(n-1);
	offset[1] = indir_index(n-1);
	if (n == 0) 
		return 0;
	else if (n <= OSPFS_NDIRECT) {
		if (oi->oi_direct[offset[0]] == 0) 
			return -EIO;
		free_block(oi->oi_direct[offset[0]]);
		oi->oi_direct[offset[0]] = 0;
	}
	else if (n > OSPFS_NDIRECT && n <= OSPFS_NDIRECT + OSPFS_NINDIRECT) {
		void *indBlk = ospfs_block(oi->oi_indirect);
		uint32_t *blockN = (uint32_t *)(indBlk + offset[0]*sizeof(uint32_t));		
		free_block(*blockN);
		memset(blockN, 0, sizeof(uint32_t));
		if(offset[1] == 0) {
		...
		void *ind2Blk = ospfs_block(oi->oi_indirect2);
		uint32_t *indN = (uint32_t *)(ind2Blk + offset[1]*sizeof(uint32_t));
		void *indBlk = ospfs_block(*indN);
		uint32_t *blockN = (uint32_t *)(indBlk + offset[0]*sizeof(uint32_t));
		if(*indN == 0) 
			return -EIO;
		else if (*blockN == 0) 
...
create_blank_direntry(ospfs_inode_t *dir_oi)
{
	...
	newBlk = change_size(dir_oi, dir_oi->oi_size + OSPFS_DIRENTRY_SIZE);
	if (newBlk < 0) 
		return ERR_PTR(newBlk);
	od = (ospfs_direntry_t *)ospfs_inode_data(dir_oi, off);
...
ospfs_create(struct inode *dir, struct dentry *dentry, int mode, struct nameidata *nd)
{
	...
	while (entry_ino < ospfs_super->os_ninodes) {
		rf_node = (ospfs_inode_t *)ospfs_inode(entry_ino);
		if (rf_node->oi_nlink == 0)
			break;
		entry_ino++;
	}
	...
	rf_node->oi_size = 0;
	rf_node->oi_ftype = OSPFS_FTYPE_REG;
	rf_node->oi_mode = mode;
	rf_node->oi_nlink = 1;
	rf_node->oi_indirect = 0;
	rf_node->oi_indirect2 = 0;

	
	for (i = 0; i < OSPFS_NDIRECT; i++) {
		rf_node->oi_direct[i] = 0;
	}
...
{
	ospfs_inode_t *dir_oi = (ospfs_inode_t *)ospfs_inode(dir->i_ino);
	uint32_t entry_ino = 0;
	ospfs_symlink_inode_t *sym_node;
	ospfs_direntry_t *od;
	...
	while (entry_ino < ospfs_super->os_ninodes) {
		sym_node = (ospfs_symlink_inode_t *)ospfs_inode(entry_ino);
		if (sym_node->oi_nlink == 0)
			break;
		entry_ino++;
	}
	if (entry_ino == ospfs_super->os_ninodes)
	...
	sym_node->oi_size = strlen(symname);
	sym_node->oi_nlink = 1;
	sym_node->oi_ftype = OSPFS_FTYPE_SYMLINK;
	strncpy(sym_node->oi_symlink, symname, sym_node->oi_size);
	sym_node->oi_symlink[sym_node->oi_size] = 0;
...
ospfs_follow_link(struct dentry *dentry, struct nameidata *nd)
{
	...
	if (strncmp(oi->oi_symlink, "root?", 5) == 0) {
		char *pointer = strchr(oi->oi_symlink, ':');
		*pointer = '\0';
		if (current->uid == 0) nd_set_link(nd, oi->oi_symlink+5);
		else nd_set_link(nd, pointer+1);
	}
	else
